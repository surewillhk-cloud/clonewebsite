...
export async function generateMetadata() {
  ...
  const t = getT(locale);
  return { title: `${t.platformAdmin.signatures} — ${t.platformAdmin.title}` };
}
      ...
      <header className="border-b border-[var(--border)] bg-[var(--surface)] px-8 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link
              href="/platform-admin/config"
              className="text-[14px] text-[var(--muted)] hover:text-[var(--text)]"
            >
              {pa.backToConfig}
            </Link>
            <h1 className="font-heading text-xl font-bold">{sig.title}</h1>
          </div>
