...
export async function generateMetadata() {
  ...
  const t = getT(locale);
  return { title: `${t.platformAdmin.tasks} — ${t.platformAdmin.title}` };
}

export default async function PlatformAdminTasksPage() {
      ...
      <header className="border-b border-[var(--border)] bg-[var(--surface)] px-8 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-heading text-xl font-bold">{pa.tasks}</h1>
          </div>
