...
export async function buildExpoProject(
  ...
  const projectDir = path.join(os.tmpdir(), `webechoai-app-${taskId}`);
...
const styles = StyleSheet.create({
  ...
  const readme = `# Cloned App - WebEcho AI

Generated by WebEcho AI from your APP screenshots.
...
`;
  ...
  const zipPath = path.join(os.tmpdir(), `webechoai-app-${taskId}.zip`);
